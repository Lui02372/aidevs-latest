...
들어갈 수 있으므로 `backend/.env.example`, `frontend/.env.example`만 전달하고, 받는 사람이
각 서비스의 `.env`를 만들게 합니다.
...
`DOCKERHUB_USERNAME`은 이메일 주소가 아니라 Docker Hub 사용자명 또는 조직명입니다.
예를 들어 사용자명이 `my-course`라면 `my-course/simple-multi-llm-backend:1.0.0` 형식을
사용합니다. 먼저 `release.env`의 두 값을 설정합니다.

```powershell
Copy-Item .\release.env.example .\release.env
# release.env에서 DOCKERHUB_USERNAME=my-course, IMAGE_TAG=1.0.0으로 수정
docker login
docker compose --env-file .\release.env build backend frontend
docker compose --env-file .\release.env push backend frontend
```

Compose가 `DOCKERHUB_USERNAME`과 `IMAGE_TAG`를 조합해 이미지 태그를 만듭니다.
`push access denied`가 나오면 로그인한 Docker Hub 사용자명과 `DOCKERHUB_USERNAME`이 같은지
확인합니다.
...
운영자가 전달한 `compose.release.yml`, `release.env.example`, `backend/.env.example`,
`frontend/.env.example`을 같은 폴더 구조로 둡니다. 각 설정 파일을 만들고 Docker Hub
사용자명과 버전을 바꿉니다.
...
```ini
DOCKERHUB_USERNAME=my-course
IMAGE_TAG=1.0.0
```

그 다음 `up`으로 Backend·Frontend Container를 실행합니다. `compose.release.yml`의
`pull_policy: always` 설정 때문에 기동할 때마다 Docker Hub에서 지정한 태그를 확인하고 받습니다.
