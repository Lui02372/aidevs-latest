...
# 01_simple-multi-llm-compose 실행·학습 가이드

## 프로젝트와 실행 순서

### 처음 시작하는 분: 여기서 순서대로 이동하세요

이 문서는 **`01_simple-multi-llm-compose` 한 폴더**의 실제 파일을 기준으로 작성했습니다. Project Player에서 해당 폴더를 선택한 뒤, 아래에서 현재 상태에 맞는 줄부터 시작하세요.

1. **9번으로 이미 서버를 켰다면:** [실행 중인 구성 확인](#running-project) → [D4 접속 확인](#check-running) → [E1 메모 저장](#practice-notes)으로 이동합니다. `build`, `up`, venv 생성부터 다시 할 필요가 없습니다. 7번은 이 문서를 만들고, 8번은 코드 주석·구조 해설을 작성하는 작업입니다.
2. **아직 서버를 실행하지 않았다면:** [PowerShell 준비](#powershell) → [환경변수](#environment) → [D1 이미지 준비](#docker-build) → D2 저장소 → D3 앱 → [D4 접속 확인](#check-running) 순서로 진행합니다. 직접 명령을 입력하는 경로와 Player의 9번 실행을 같은 앱에 중복 사용하지 않습니다.
3. **화면과 저장소를 이해합니다:** [E1](#practice-notes)에서 메모 저장·조회와 API 응답을 확인합니다. API 키는 필요하지 않습니다.
4. **키 없이 실제 AI 답변을 받습니다:** [E2 Ollama](#practice-ollama)에서 현재 실행 방식에 맞는 Ollama를 준비하고, 화면의 Provider를 `ollama`로 선택합니다. 이미 Ollama 채팅이 된다면 설치·재생성 명령을 반복하지 말고 질문·대화 이력 실습으로 이동합니다.
5. **코드 검증과 자동화를 배웁니다:** [GitHub Actions](#github-actions)에서 이미 있는 `07 Runtime CI`의 테스트 → Compose 검사 → 이미지 빌드 순서를 읽고, 필요한 경우 로컬 테스트를 수행합니다. GitHub 연결은 로컬 서버 실행의 선행 조건이 아닙니다.
6. **학습을 마칠 때만 종료합니다:** Player로 시작했다면 Player의 서버 중단 기능을 사용합니다. 직접 Compose로 시작했다면 [D5 종료](#stop-project)의 해당 구성 명령을 사용합니다. 다음 실습을 이어갈 때는 서버를 켜 둡니다.

각 단계의 **목적 → 작업 폴더·창 → 명령 → 예상 결과 → 실패 시 확인**을 한 단계씩 확인하세요. 오류가 있으면 [검증과 문제 해결](#verification)로 이동합니다.

<a id="running-project"></a>

### 9번으로 이미 실행 중일 때: 현재 구성부터 확인

Project Player의 **서버 실행 중** 표시와 접속 버튼을 먼저 봅니다. 분석에서 `compose.yml`을 찾았더라도 실행기가 저장소 준비 상태에 따라 `compose.full-stack.yml`을 사용했을 수 있으므로, 파일 이름만 보고 실행 구성을 단정하지 않습니다.

PowerShell로도 확인하려면 다음 **조회 명령만** 입력합니다. 실행·재시작 명령이 아닙니다.
...
행이 표시되는 구성과 `PORTS`를 확인합니다. Full Stack이면 프론트는 `8501`, 기본 구성이면 `80`이며 Backend는 둘 다 `8000`입니다. `healthy` 또는 실행 상태를 확인했다면 바로 [D4](#check-running)와 [E1](#practice-notes)로 이동하세요. 아직 준비 중이면 Player의 준비 결과를 기다립니다. 이 문서 작성에서는 현재 컨테이너 상태를 새로 검사하지 않았으므로 두 구성 중 하나가 실행 중이라고 단정하지 않습니다.
...
Player에서 이미 실행했다면 위의 [실행 중인 구성 확인](#running-project)을 따라 같은 앱의 중복 실행을 피하고 확인·실습 단계부터 진행하세요.
...
| Python 3.12 | 컨테이너는 두 Dockerfile의 `python:3.12-slim` 사용. Windows Python은 공용 DB 초기화 또는 로컬 계약 테스트에 필요 | 호스트 설치 미확인 |
| pip·venv | 호스트에서 `init_database.py`나 로컬 계약 테스트를 실행할 때 필요 | 루트 `.venv`는 최초 문서 확인 당시 없음; 현재 상태는 재확인 필요 |
...
가상환경 활성화는 필요하지 않습니다. 이후에도 가상환경 Python을 직접 호출합니다. 앱 서버의 로컬 시작·확인·종료는 다음 Docker 절의 명령을 사용합니다. [GitHub Actions](#github-actions)의 계약 테스트를 PC에서 실행할 때는 같은 venv에 Backend 의존성과 `pytest`를 추가합니다.

**venv와 Docker의 역할:** 이 문서에서 앱 서버는 Docker로 실행하며, Host venv는 초기화 도구·테스트용입니다. Backend·Frontend는 `os.getenv`로 값을 읽을 뿐 `.env`를 자동 로드하지 않습니다. 또한 Full Stack의 DB·Redis는 Host 포트를 공개하지 않습니다. 따라서 `.env` 복사와 venv 생성만으로 Host Python 앱이 Full Stack 저장소에 바로 연결된다고 해석하면 안 됩니다.
...
**처음 수동으로 Full Stack을 준비하며 메모만 실습한다면** PostgreSQL 계정 정보를 확인하고 `OLLAMA_ENABLED=false`로 시작할 수 있습니다. 예제 파일에는 `true`가 적혀 있지만 Ollama 프로필 실행과 모델 다운로드는 별도 단계입니다. API 키가 없어도 메모 실습은 가능합니다. **9번으로 이미 서버나 Ollama가 정상 실행 중이라면 이 단계를 위해 값을 바꾸거나 컨테이너를 재생성하지 마세요.** 바로 [E1](#practice-notes) 또는 [E2](#practice-ollama)의 화면 실습으로 이동합니다.
...
| `POSTGRES_PASSWORD` | 같은 위치, `backend.environment.DATABASE_URL` | 새 DB 비밀번호와 Backend 접속 인증 | 예제의 로컬 학습용 값 `agent_pwd` | Full Stack에서 필요 | 새 학습 볼륨은 예제값 사용 가능; 기존 볼륨은 생성 당시 값과 일치해야 함 |
| `POSTGRES_DB` | `.env.example`, Full Stack의 `database.environment` | 사용할 데이터베이스 이름 | 개발 예시 `agent_db` | Full Stack에서 필요 | 기존 볼륨이면 실제 DB 이름 |
| `DATABASE_URL` | 기본·Release Compose, `PostgresRepository.__init__` | Backend의 PostgreSQL 접속 문자열 | 학습 예제 `postgresql://agent_user:agent_pwd@host.docker.internal:5433/agent_db` | 기본·Release에서 실제 DB와 일치해야 함 | 공용 DB가 다른 계정·포트를 쓰면 그 접속 URL |
| `HOST_DATABASE_URL` | `.env.example`, `init_database.py:host_database_url()` | Windows에서 초기화 도구가 접속할 DB 주소 | 학습 예제 `postgresql://agent_user:agent_pwd@127.0.0.1:5433/agent_db` | 초기화 시 선택, 지정하면 우선 사용 | 공용 DB의 Host용 접속 URL |
...
아래 화면 검사는 Full Stack·Release의 `8501`을 사용합니다. [현재 구성 확인](#running-project)에서 기본 `compose.yml`이 실행 중이었다면 `$frontendUrl`을 `http://127.0.0.1`로 바꾸세요. Backend 확인 주소는 동일합니다.
...
Invoke-WebRequest -UseBasicParsing -Uri $frontendUrl |
...
위에서 확인한 `$frontendUrl`을 브라우저로 열고 [E1 메모 실습](#practice-notes)으로 진행합니다.
...
### 이미 있는 CI 파일과 정확한 적용 범위

**현재 CI 파일이 있습니다.** 선택한 미니 프로젝트 안에는 `.github/workflows`가 없지만, 세 단계 위의 상위 소스 폴더에 있는 [07-runtime-ci.yml](../../../.github/workflows/07-runtime-ci.yml)을 직접 읽어 확인했습니다.

```text
C:\수업 보관소\aidevs-latest\aidevs-main\aidevs-main\
├─ .github/workflows/07-runtime-ci.yml
└─ 07_multi-agent-service-ops/00_runtime-and-deployment/
   └─ 01_simple-multi-llm-compose/   ← 지금 선택한 프로젝트
```

Git 메타데이터 확인 실패 또는 로컬 폴더가 Git checkout이 아닌 상태는 **이 YAML 파일의 부재를 뜻하지 않습니다.** 파일 내용은 확인했으며, 연결된 원격 저장소·현재 브랜치·GitHub의 Actions 허용 여부·실제 실행 결과는 아직 확인하지 않았습니다.

| 항목 | 현재 파일에서 확인한 값 |
| --- | --- |
| 표시 이름 | `07 Runtime CI` |
| 파일 경로 | 상위 소스 폴더의 `.github/workflows/07-runtime-ci.yml` |
| 자동 실행 이벤트 | `push`, `pull_request` |
| 경로 조건 | `07_multi-agent-service-ops/00_runtime-and-deployment/**` 또는 `.github/workflows/07-runtime-ci.yml` 변경 |
| 브랜치 조건 | 이 workflow에 별도의 `branches` 필터 없음 |
| 수동 실행 | `workflow_dispatch` 있음 |
| job | `test-and-build` 한 개; 다른 job을 기다리는 `needs` 없음 |
| 실행 환경 | `ubuntu-latest`, Python `3.12` |
| 작업 폴더 | `07_multi-agent-service-ops/00_runtime-and-deployment/01_simple-multi-llm-compose` |
| 권한 | `contents: read` |
| 단계 순서 | Checkout → Python 준비 → 테스트 의존성 설치 → Backend 계약 테스트 → Compose 검사 → 이미지 빌드 |
| Secret·Variable 참조 | 이 workflow에는 없음 |
| Registry 게시·EC2 배포 | 이 workflow에는 없음 |

`actions/checkout@v4`, `actions/setup-python@v5`는 현재 YAML에 적힌 버전입니다. 최신 버전이라는 의미가 아니라, 이 수업 파일이 무엇을 실행하도록 작성됐는지를 보여 줍니다.

상위의 `07-weather-mcp-cicd.yml`과 `07-weather-stateful-cicd.yml`도 존재하지만 각각 **05·06 Weather 프로젝트 배포용**입니다. 거기에 있는 AWS SSH Secrets를 이 `01` 프로젝트의 필수 설정으로 복사하지 않습니다.

### CI가 하는 일을 PC에서 따라 해 보기

**목적:** 서버를 다시 켜는 작업과 코드 검사·이미지 빌드를 구분합니다. `backend/test_app.py`는 Redis·DB·LLM을 Fake 객체로 바꾸므로 실제 API 키와 실행 중인 저장소 없이 계약을 검사합니다.

**작업 폴더·창:** 프로젝트 루트, A창. Docker 서버를 실행 중이어도 아래 테스트가 기존 컨테이너를 중단하거나 다시 만들지는 않습니다. Windows에서 테스트를 할 때만 Python 3.12와 venv가 필요합니다.
...
**예상 결과:** 현재 파일의 계약 테스트 세 개가 통과하며 `3 passed`가 표시됩니다. 실패하면 어떤 assertion 또는 import가 실패했는지 먼저 확인합니다. `pytest`는 앱 requirements에 없어서 CI와 위 명령이 따로 설치합니다. 이 문서 작성 중 테스트를 실행한 것은 아닙니다.

검사가 통과했다면 다음 두 명령으로 CI의 나머지를 따라 합니다.
...
**예상 결과:** 첫 명령은 Compose 구문·환경변수 참조를 검사하고, 두 번째는 Backend·Frontend 이미지를 빌드합니다. CI는 기본 `compose.yml`을 검사합니다. Full Stack을 실행 중이더라도 이 두 명령은 다른 컨테이너를 시작하지 않습니다. 로컬 `.env` 때문에 치환되는 값이 있으면, `.env`를 업로드하지 않는 GitHub 실행과 결과가 다를 수 있습니다.

CI 통과는 **Fake 계약·Compose 문법·이미지 빌드** 성공을 뜻합니다. 실제 DB 연결·모델 응답·사용자 PC의 포트 상태는 [D4](#check-running)와 [E1·E2](#practice-notes)로 별도 확인합니다.

### GitHub에서 무엇을 연결하고 확인할까

로컬 서버 실행에 GitHub 연결은 필수가 아닙니다. CI를 실제 GitHub에서 실행할 때만 다음 순서로 진행합니다.

1. **사용할 원격 저장소를 확인합니다.** 원본 저장소 또는 본인이 관리하는 저장소의 URL과 접근 권한을 확인합니다. 현재 로컬 폴더가 ZIP 사본인지 Git checkout인지는 이번 확인으로 단정하지 않습니다.
2. **폴더 배치를 확인합니다.** GitHub 저장소 루트에 `.github/workflows/07-runtime-ci.yml`이 있고, 위 표의 `working-directory` 위치에 미니 프로젝트가 있어야 합니다. `01_simple-multi-llm-compose`만 새 저장소 루트에 올릴 경우에는 YAML의 `working-directory`와 `paths`를 그 배치에 맞게 수정해야 합니다.
3. **저장소의 Actions 허용 여부를 확인합니다.** `Settings → Actions → General`의 정책과 본인 권한을 봅니다. 이 workflow는 별도의 API 키·AWS 키·Registry 비밀번호를 요구하지 않습니다.
4. **실행 조건을 이해합니다.** 해당 경로를 변경한 push·PR에서 자동 실행됩니다. 로컬 `git pull`은 GitHub의 push 이벤트가 아닙니다. 기본 브랜치에 `workflow_dispatch`가 있는 파일이 반영되고 실행 권한이 있다면 `Actions → 07 Runtime CI → Run workflow`에서 수동 실행할 수 있습니다.
5. **결과를 확인합니다.** `test-and-build`에서 세 계약 테스트, Compose 검사, 이미지 빌드 단계가 각각 성공했는지 확인합니다. 이번 문서 작성에서는 저장소 등록·push·수동 실행을 수행하지 않았습니다.

`BACKEND_IMAGE`·`FRONTEND_IMAGE`는 현재 **로컬 release Compose 변수**입니다. GitHub Variables가 아니며 이 CI는 해당 값을 참조하지 않습니다. 이미지 게시나 EC2 배포를 추가하려면 게시 대상·계정·인증·배포 job을 별도로 설계해야 합니다. 현재 `01` 실습을 실행하기 위한 필수 작업은 아닙니다.

### Actions 실패를 읽는 순서

**Actions → 07 Runtime CI → 해당 run → test-and-build → 실패 step → 로그** 순서로 봅니다.

- 시작되지 않음: workflow가 GitHub에 존재하는지, Actions 정책, push·PR의 변경 경로가 `paths`와 일치하는지 확인합니다.
- 경로 실패: 저장소 루트의 폴더 배치와 YAML의 `working-directory`를 대조합니다.
- 의존성 실패: Python 3.12, `backend/requirements.txt`, 네트워크·패키지 설치 오류를 확인합니다.
- 테스트 실패: `backend/test_app.py`의 실패 assertion과 Fake 의존성을 확인합니다. API 키를 추가해 Fake 테스트를 통과시키는 구조가 아닙니다.
- Docker 실패: `backend`·`frontend` 중 어떤 Dockerfile의 `COPY` 또는 패키지 설치가 실패했는지 확인합니다.

현재 파일의 정의는 확인했지만, 원격 저장소의 실제 run 성공 여부는 미확인입니다.
...
**작업 폴더·창:** 브라우저는 [D4](#check-running)에서 확인한 화면 주소입니다. Full Stack·Release는 `http://127.0.0.1:8501`, 기본 Compose는 `http://127.0.0.1`입니다. 확인 명령은 프로젝트 루트의 A창에서 실행합니다.
...
<a id="practice-ollama"></a>

### E2. Ollama로 채팅하고 대화 저장 확인하기

먼저 [현재 실행 구성](#running-project)을 확인합니다. **Ollama의 준비 명령은 Full Stack과 공용 서비스 방식이 다릅니다.** 이미 Provider `ollama`로 실제 답변을 받고 있다면 아래 준비·컨테이너 재생성은 건너뛰고 질문·이력 확인 실습만 수행하세요.

#### 기본 Compose·Release라면: 공용 Ollama 사용

이 경로의 Backend는 `http://host.docker.internal:11434`를 사용합니다. **기본 앱을 실행한 상태에서 아래 Full Stack `up` 명령을 추가로 실행하지 않습니다.**

프로젝트 루트의 A창에서 공용 Ollama 상태와 모델 목록을 조회합니다.
...
**예상 결과:** 포트 연결이 성공하고 모델 목록에 `gemma3:4b`가 있습니다. 모델이 없고 Windows의 Ollama CLI를 사용하는 공용 환경이라면 `ollama pull gemma3:4b`로 준비합니다. 공용 Ollama가 별도 Docker 컨테이너라면 그 컨테이너의 관리 절차로 모델을 준비해야 합니다. 이 프로젝트의 기본 Compose는 공용 Ollama를 생성·관리하지 않습니다. 기존 공용 환경이 없다면 기본 앱을 중단한 뒤 [Full Stack 경로](#docker-build) 한 가지로 전환합니다.

`.env`에서 `OLLAMA_ENABLED=true`, `OLLAMA_BASE_URL=http://host.docker.internal:11434`, `GEMMA_MODEL=gemma3:4b`를 확인합니다. 값을 바꾼 경우에만 **현재 기본 구성**에 반영합니다.

```powershell
docker compose -f '.\compose.yml' up -d --force-recreate backend frontend
```

Release로 실행 중이라면 위 `-f`를 `.\compose.release.yml`로 바꿉니다. 이후 [화면에서 채팅하기](#ollama-chat)로 이동합니다. 값이 이미 올바르고 채팅이 된다면 재생성하지 않습니다.

#### Full Stack이라면: 내부 Ollama 컨테이너 사용
...
다음 모델로 비교하려면 같은 A창에서 Llama를 다운로드합니다. 아래는 **Full Stack용**이며, 공용 Ollama 경로는 앞서 확인한 공용 모델 관리 방식으로 `llama3.2`를 준비합니다.
...
Full Stack 실습 종료는 [Docker D5](#stop-project)를 따릅니다. 기본 Compose·Release는 자신이 시작한 구성으로 종료하고, Project Player로 시작했다면 Player의 서버 중단 기능을 사용합니다.
...
| 기존 사용자 안내 | `guide.md` |
| GitHub Actions | 상위 소스 폴더의 `.github/workflows/07-runtime-ci.yml`: `on`, `paths`, `test-and-build`, `working-directory`, Python 3.12, 계약 테스트·Compose 검사·이미지 빌드 |
...
- `.env`는 최초 확인 당시 존재했고 루트 `.venv`는 없었습니다. 현재 생성 여부와 내용은 이번 보완에서 다시 확인하지 않았습니다.
- 선택 폴더와 확인한 상위 경로에서 적용할 로컬 `AGENTS.md` 파일은 발견하지 못했습니다. 대화로 제공된 작업 지침을 적용했습니다.
- 선택 폴더에는 별도 `SETUP.md`, `package.json`, 확인 대상 잠금 파일, `.github/workflows`가 없습니다.
- 상위 `aidevs-main/.github/workflows/07-runtime-ci.yml` 파일은 존재하며, `working-directory`가 이 미니 프로젝트를 정확히 가리키는 것을 직접 읽어 확인했습니다.
- 상위의 두 Weather 배포 workflow는 각각 05·06 프로젝트를 가리킵니다. 현재 01 프로젝트의 배포 정의가 아닙니다.
- Git 메타데이터·원격 저장소 확인이 실패하거나 미완료된 사실과 CI 파일의 존재를 구분합니다. 원격 URL, 현재 브랜치, GitHub에서의 실행 결과는 미확인입니다.
...
이 문서는 소스·설정·상위 CI 파일을 읽어 작성·보완한 실행 안내입니다. 문서 작성 자체가 서버 준비 완료를 뜻하지 않습니다. 이 보완 작업에서 앱 import, 패키지 설치, Python 스크립트·테스트, Docker 명령, 서버 시작·종료, DB 변경, 이미지 게시, workflow 실행과 배포는 수행하지 않았습니다.
...
- CI를 GitHub에서 실행할 경우 사용할 원격 저장소·권한과 현재 workflow의 경로 배치 유지 여부
