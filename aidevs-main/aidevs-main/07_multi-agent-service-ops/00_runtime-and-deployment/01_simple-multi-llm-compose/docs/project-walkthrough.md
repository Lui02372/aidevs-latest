# 프로젝트 코드 따라 읽기: Multi-LLM 여행 준비 앱
...
채팅 외에 “여행 메모” 탭에서 메모 저장·조회, “서비스 상태” 탭에서 상태 조회를 할 수 있다. 화면에 있는 기능은 [`frontend/app.py:53`](../frontend/app.py#L53), [`frontend/app.py:77`](../frontend/app.py#L77), [`frontend/app.py:105`](../frontend/app.py#L105), [`frontend/app.py:121`](../frontend/app.py#L121)에서 확인했다. 모델의 여행 도우미 안내문은 [`backend/services.py:187`](../backend/services.py#L187)에 있다.
...
| --- | --- | --- |
| Python 3.12 컨테이너 | [`backend/Dockerfile:2`](../backend/Dockerfile#L2), [`frontend/Dockerfile:2`](../frontend/Dockerfile#L2) | 두 앱의 실행 환경. 이미지가 실제 이 버전으로 실행 중인지는 확인하지 않았다. |
| Backend 패키지 | [`backend/requirements.txt:2`](../backend/requirements.txt#L2)부터 14행 | FastAPI·Uvicorn, Redis·Psycopg, google-genai·openai, httpx. 모두 범위 명세라 정확한 설치 버전 목록은 아니다. |
| Frontend 패키지 | [`frontend/requirements.txt:2`](../frontend/requirements.txt#L2), [`frontend/requirements.txt:4`](../frontend/requirements.txt#L4) | Python 화면과 API 요청에 필요하다. Node 런타임이나 npm 설치는 앱에 필요하지 않다. |
| Host venv | 루트 `.venv`·`venv` 없음 | Host에서 패키지를 격리하는 폴더다. 컨테이너로 앱을 실행할 때 Host venv가 반드시 필요한 것은 아니다. |
| 별도 초기화 도구 | [`init_database.py:18`](../init_database.py#L18), [`init_database.py:19`](../init_database.py#L19) | Host에서 공용 DB를 준비할 때 사용한다. `python-dotenv`는 Backend 명세에 없고 `pytest`도 명세에 없다. |
...
Dockerfile은 `requirements.txt`를 먼저 복사·설치한 후 앱 소스를 복사한다. 명세가 그대로라면 이미지 빌드에서 패키지 계층을 재사용할 수 있다. `CMD`는 이미지 생성 때 실행하는 코드가 아니라 컨테이너 시작 때 실행할 기본 명령이다. Backend의 `app:app`은 **`app.py` 모듈 안의 `app` 객체**를 뜻한다. 근거: [`backend/Dockerfile:6`](../backend/Dockerfile#L6), [`backend/Dockerfile:12`](../backend/Dockerfile#L12), [`frontend/Dockerfile:12`](../frontend/Dockerfile#L12).

### 세 Compose 구성의 경계

| 구성 | 앱 공급 방식 | 저장소·모델 연결 설정 | Host에서 화면 / API | 근거 |
| --- | --- | --- | --- | --- |
| `compose.yml` | 두 앱을 로컬 소스로 빌드 | 기본값은 `host.docker.internal`의 Redis 6379·DB 5433·Ollama 11434. 환경변수로 변경 가능 | **80 → 8501** / 8000 → 8000 | [`compose.yml:10`](../compose.yml#L10), [`compose.yml:13`](../compose.yml#L13), [`compose.yml:54`](../compose.yml#L54) |
| `compose.full-stack.yml` | 두 앱 빌드 + Redis·DB + 선택 Ollama 이미지 | 네트워크 안의 `redis:6379`, `database:5432`, `ollama:11434`. 저장소 Host 포트 공개 없음 | 8501 → 8501 / 8000 → 8000 | [`compose.full-stack.yml:5`](../compose.full-stack.yml#L5), [`compose.full-stack.yml:59`](../compose.full-stack.yml#L59), [`compose.full-stack.yml:99`](../compose.full-stack.yml#L99) |
| `compose.release.yml` | `BACKEND_IMAGE`, `FRONTEND_IMAGE`로 기존 이미지 지정 | 기본 구성과 같이 공용 서비스로 연결 | 8501 → 8501 / 8000 → 8000 | [`compose.release.yml:11`](../compose.release.yml#L11), [`compose.release.yml:50`](../compose.release.yml#L50), [`compose.release.yml:56`](../compose.release.yml#L56) |
...
| --- | --- | --- |
| `OPENAI_API_KEY`, `GEMINI_API_KEY` | `configured()`가 설정 존재 확인 | [`backend/services.py:155`](../backend/services.py#L155) → 선택한 SDK 분기 |
| `OPENAI_MODEL`, `GEMINI_MODEL` | 공급자 모델명 선택 | [`backend/services.py:166`](../backend/services.py#L166) |
| `OLLAMA_ENABLED` | 문자열 플래그를 참/거짓으로 해석 | `configured()`; Ollama 프로필·모델 다운로드와 별개 |
| `OLLAMA_BASE_URL` | Ollama HTTP 서버 주소 | [`backend/services.py:233`](../backend/services.py#L233) |
| `OLLAMA_MODEL`, `GEMMA_MODEL` | `ollama_model` 선택에 따른 모델명 | `model()` → `/api/chat` 요청 본문 |
| `REDIS_URL` | Redis 접속 대상 | [`backend/services.py:32`](../backend/services.py#L32) |
| `DATABASE_URL` | Backend PostgreSQL 접속 대상 | [`backend/services.py:83`](../backend/services.py#L83) |
| `HOST_DATABASE_URL` | 별도 초기화 프로그램에서 우선 사용할 접속 설정 | [`init_database.py:34`](../init_database.py#L34) |
| `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB` | Full Stack DB 초기 생성 및 Backend 접속 구성 | [`compose.full-stack.yml:28`](../compose.full-stack.yml#L28) |
| `BACKEND_IMAGE`, `FRONTEND_IMAGE` | release 이미지와 버전 선택 | `compose.release.yml`의 `image` 키 |
| `BACKEND_URL` | Streamlit에서 API 주소 구성 | [`frontend/app.py:14`](../frontend/app.py#L14) |

일반 앱 모듈은 `.env`를 직접 읽는 코드를 갖고 있지 않다. Compose가 환경변수를 컨테이너에 전달한다. `init_database.py`만 `load_dotenv`를 호출한다. 공용 DB는 이 폴더의 SQL을 자동으로 실행하지 않으며, Full Stack은 SQL을 초기화 경로에 읽기 전용으로 연결한다. **기존 볼륨에 테이블이 없다고 컨테이너 시작만으로 SQL이 재실행되는 것은 아니다.** 근거: [`init_database.py:36`](../init_database.py#L36), [`compose.full-stack.yml:35`](../compose.full-stack.yml#L35).
...
| --- | --- | --- |
| B·F·A, B→F→A | [`frontend/app.py:86`](../frontend/app.py#L86), [`frontend/app.py:90`](../frontend/app.py#L90) | 입력 조건과 실제 네 필드 POST |
| A→V | [`frontend/app.py:18`](../frontend/app.py#L18), [`backend/app.py:151`](../backend/app.py#L151) | `httpx.request`에서 FastAPI 경로로 연결 |
| V→C, V→E | [`backend/app.py:26`](../backend/app.py#L26), [`backend/app.py:54`](../backend/app.py#L54), [`backend/app.py:152`](../backend/app.py#L152) | Pydantic 입력 검증, 의존성 생성 함수 주입 |
| C→E·R | [`backend/app.py:154`](../backend/app.py#L154), [`backend/app.py:159`](../backend/app.py#L159) | 설정 확인 후에만 Redis 접근 |
| R·R→U | [`backend/services.py:58`](../backend/services.py#L58), [`backend/app.py:160`](../backend/app.py#L160) | Redis JSON 복원 후 사용자 기록 저장 |
| U·U→S | [`backend/services.py:129`](../backend/services.py#L129), [`backend/app.py:162`](../backend/app.py#L162) | DB 커밋 뒤 서비스 호출 순서 |
| S | [`backend/services.py:200`](../backend/services.py#L200), [`backend/services.py:187`](../backend/services.py#L187), [`backend/services.py:166`](../backend/services.py#L166) | 문맥 6개로 프롬프트와 모델명 구성 |
| S→O, O→P | [`backend/services.py:220`](../backend/services.py#L220), [`backend/services.py:247`](../backend/services.py#L247), [`backend/app.py:170`](../backend/app.py#L170) | OpenAI 분기 반환값으로 답변 저장 |
| S→G, G→P | [`backend/services.py:227`](../backend/services.py#L227), `LLMReply`, `chat`의 assistant 저장 | Gemini 분기도 같은 반환 계약 |
| S→L, L→P | [`backend/services.py:235`](../backend/services.py#L235), [`backend/services.py:242`](../backend/services.py#L242), `chat`의 assistant 저장 | Ollama JSON에서 내용 추출, 스트림 비활성 |
| S→E | [`backend/app.py:179`](../backend/app.py#L179), [`backend/services.py:244`](../backend/services.py#L244) | 외부 호출·빈 응답 예외가 503으로 변환 |
| P·P→W | [`backend/app.py:171`](../backend/app.py#L171), [`backend/services.py:64`](../backend/services.py#L64) | 질문·답변 영구 저장 뒤 최근 세션 갱신 |
| W→J | [`backend/app.py:176`](../backend/app.py#L176), [`backend/app.py:183`](../backend/app.py#L183), [`backend/services.py:44`](../backend/services.py#L44) | 채팅의 통계 갱신과 최종 dict 응답. |
| J→D, E→D, D→B | [`frontend/app.py:22`](../frontend/app.py#L22), [`frontend/app.py:98`](../frontend/app.py#L98), [`frontend/app.py:27`](../frontend/app.py#L27) | 정상 답변과 오류를 화면에 표시 |
| 실제 테이블 P·U | [`database/init.sql:16`](../database/init.sql#L16) | 저장 열은 session_id·role·content·created_at 등 |
...
| --- | --- | --- |
| POST 전에 기록 GET | [`frontend/app.py:79`](../frontend/app.py#L79), [`backend/app.py:193`](../backend/app.py#L193), [`backend/services.py:138`](../backend/services.py#L138) | 재실행 때 기존 이력을 그린 다음 제출값을 처리한다. 다이어그램은 기록 조회가 성공한 경우를 선택했다. |
| POST 검증과 객체 주입 | [`backend/app.py:26`](../backend/app.py#L26), [`backend/app.py:54`](../backend/app.py#L54), [`backend/app.py:152`](../backend/app.py#L152) | 길이·허용 문자열·정규식 검사. 인증 단계는 없다. |
| 미설정 503 | [`backend/app.py:154`](../backend/app.py#L154), [`backend/services.py:155`](../backend/services.py#L155) | 저장소에 접근하기 전 반환한다. 실제 API 키 유효성 검사는 아니다. |
| 문맥 읽기·질문 저장 | [`backend/app.py:159`](../backend/app.py#L159), [`backend/services.py:129`](../backend/services.py#L129) | DB 저장은 메서드마다 별도 `with` 연결이다. |
| 모델 요청·응답 | [`backend/services.py:200`](../backend/services.py#L200)부터 247행 | 분기 하나만 호출하고 `LLMReply`로 통일한다. |
| 모델 오류 전파 | [`backend/services.py:241`](../backend/services.py#L241), [`backend/services.py:244`](../backend/services.py#L244), [`backend/app.py:179`](../backend/app.py#L179) | 이미 저장한 user 행을 되돌리는 코드가 없다. |
| 답변 저장·Redis 갱신·응답 | [`backend/app.py:170`](../backend/app.py#L170)부터 188행 | 여기서 Redis가 실패하면 두 DB 행이 남아도 전체 응답은 503이다. |
| 성공·오류 화면 | [`frontend/app.py:98`](../frontend/app.py#L98), [`frontend/app.py:27`](../frontend/app.py#L27) | 이번에는 실제 모델 질문 제출이나 오류 유발 요청을 보내지 않았다. |
...
| --- | --- | --- | --- |
| GET `/health/live` | 없음 → status·service | 외부 의존성 없이 200 | [`backend/app.py:61`](../backend/app.py#L61) |
| GET `/health` | 없음 → status·checks·errors | Redis PING, DB SELECT·테이블 존재, 공급자 설정. degraded여도 기본 200 | [`backend/app.py:67`](../backend/app.py#L67) |
| GET `/health/ready` | 없음 → health 결과 | 저장소·스키마가 준비되지 않으면 503 | [`backend/app.py:104`](../backend/app.py#L104) |
| POST `/api/notes` | name·message → note·request_count·warning | 성공 201. DB 실패 503. DB 성공 후 Redis 실패는 201+warning | [`backend/app.py:115`](../backend/app.py#L115) |
| GET `/api/notes` | 없음 → notes 목록 | 최신 50개 기본값. DB 실패 503 | [`backend/app.py:134`](../backend/app.py#L134) |
| GET `/api/stats` | 없음 → request_count·recent_request | Redis 실패 503. 현재 UI 호출 없음 | [`backend/app.py:143`](../backend/app.py#L143) |
| POST `/api/chat` | session_id·message·provider·ollama_model → 답변 JSON | 검증 실패 422, 미설정·처리 실패 503, 성공 200 | [`backend/app.py:152`](../backend/app.py#L152) |
| GET `/api/chat/{session_id}` | 경로 문자열 → session_id·messages | 오래된 순서 최대 100개. DB 실패 503 | [`backend/app.py:193`](../backend/app.py#L193) |
| DELETE `/api/sessions/{session_id}` | 경로 문자열 → 삭제/보존 표시 | Redis 키만 삭제. DB 이력 보존, Redis 실패 503 | [`backend/app.py:202`](../backend/app.py#L202) |
...
| --- | --- | --- | --- |
| PostgreSQL `simple_multi_llm.notes` | id, name, message, created_at | BIGSERIAL PK, 길이 제한, NOT NULL, DB 기본 시각 | [`database/init.sql:5`](../database/init.sql#L5), [`backend/services.py:107`](../backend/services.py#L107) |
| PostgreSQL `simple_multi_llm.chat_messages` | id, session_id, role, content, created_at | role은 user/assistant만. 세션·id 복합 인덱스. 이 코드의 삭제 경로 없음 | [`database/init.sql:16`](../database/init.sql#L16), [`database/init.sql:28`](../database/init.sql#L28) |
| Redis 세션 키 | `runtime_demo:session:{session_id}` | JSON 문자열 리스트, 마지막 12개, append마다 1800초 TTL 갱신 | [`backend/services.py:64`](../backend/services.py#L64) |
| Redis 누적 요청 키 | `runtime_demo:request_count` | 메모·채팅 성공 흐름의 공용 횟수. 이 코드에서는 TTL 없음 | [`backend/services.py:44`](../backend/services.py#L44) |
| Redis 최근 요청 키 | `runtime_demo:recent_request` | 최근 질문·메모 문자열, TTL 1800초 | 같은 `record_request` |
| Streamlit 상태 | `st.session_state.session_id` | 같은 화면 세션의 재실행 간 유지. 브라우저 재접속 때 동일 ID를 복원하는 기능 없음 | [`frontend/app.py:48`](../frontend/app.py#L48) |
...
| --- | --- | --- | --- | --- |
| `from __future__ import annotations` · [`backend/services.py:2`](../backend/services.py#L2) | 타입 설명의 평가를 미룬다 | `def f(x: Later): ...` | 타입 이름을 정의 시점에 바로 계산하지 않음 | 타입 힌트가 모든 런타임 값을 자동 검사한다고 생각함 |
| `url: str \| None = None` · [`backend/services.py:30`](../backend/services.py#L30) | 문자열 또는 값 없음, 생략 기본값은 None | `x: int \| None = None` | 인수·환경변수·기본 주소 우선순위 | 빈 문자열과 None의 차이를 무시함 |
| `Literal[...]`, `Field(...)` · [`backend/app.py:26`](../backend/app.py#L26) | 허용 선택과 길이·형식을 정한다 | `Literal["a", "b"]` | API 입력 경계를 좁힘 | 다른 GET 경로의 단순 str에도 같은 제약이 있다고 생각함 |
| `@app.post(...)` · [`backend/app.py:151`](../backend/app.py#L151) | 함수에 HTTP 경로를 등록한다 | `@app.get("/hello")` | 요청을 `chat()`에 연결 | 함수 이름만 같으면 경로도 같다고 생각함 |
| `@lru_cache` · [`backend/app.py:36`](../backend/app.py#L36) | 동일 인수의 함수 결과를 재사용한다 | `cached_factory()` | 생성된 저장소 객체 재사용 | 모델 응답이나 세션 데이터까지 자동 캐시된다고 생각함 |
| `Annotated[..., Depends(...)]` · [`backend/app.py:54`](../backend/app.py#L54) | 타입과 객체 공급 규칙을 함께 적는다 | `Depends(make_store)` | FastAPI가 객체를 주입하고 테스트는 교체 가능 | `make_store()`를 미리 실행한 값과 함수 자체를 혼동함 |
| `@dataclass(frozen=True)` · [`backend/services.py:20`](../backend/services.py#L20) | 필드를 가진 재대입 방지 객체 | `Reply(text="안녕")` | 공급자별 답변 형식을 통일 | DB에 자동 저장되는 클래스라고 생각함 |
| `@staticmethod` · [`backend/services.py:186`](../backend/services.py#L186) | 객체 상태 없이 쓰는 도우미 | `Formatter.join(items)` | `_prompt`는 전달받은 값만 필요 | 불필요한 self 인수를 전달함 |
| `[json.loads(item) for item in ...]` · [`backend/services.py:61`](../backend/services.py#L61) | 목록의 각 항목을 변환한다 | `[n * 2 for n in [1, 2]]` → `[2, 4]` | Redis 문자열을 메시지 dict로 복원 | JSON 문자열과 dict를 같은 것으로 취급함 |
| `{name: ... for name in ...}` · [`backend/app.py:75`](../backend/app.py#L75) | 이름별 결과 사전을 만든다 | `{x: True for x in ["a", "b"]}` | 공급자 설정 상태를 일괄 구성 | 이 계산이 실제 모델 호출이라고 생각함 |
| `*[json.dumps(...) ...]` · [`backend/services.py:69`](../backend/services.py#L69) | 목록을 여러 인수로 펼친다 | `f(*[1, 2])`는 `f(1, 2)` | RPUSH에 메시지 여러 개 전달 | `**`도 목록을 펼친다고 생각함. `**`는 매핑용이며 여기엔 없음 |
| `recent_messages[-6:]`, `join(...)` · [`backend/services.py:189`](../backend/services.py#L189) | 마지막 여섯 개를 줄로 이어 붙인다 | `"\n".join(str(n) for n in [1, 2])` | 프롬프트 길이를 제한 | 여섯 메시지를 여섯 대화 쌍으로 오해함 |
| `with ... as connection, ... as cursor` · [`backend/services.py:91`](../backend/services.py#L91) | 자원 사용 범위를 만들고 나갈 때 정리한다 | `with open("sample.txt") as f: ...` | DB 연결·트랜잭션·커서 수명 관리 | 서로 다른 메서드의 with까지 한 트랜잭션으로 생각함 |
| `(limit,)` · [`backend/services.py:123`](../backend/services.py#L123) | 원소가 하나인 튜플이다 | `(3,)`은 튜플, `(3)`은 정수 | SQL 매개변수 묶음 전달 | 마지막 쉼표를 지움 |
| `raise ... from error` · [`backend/app.py:121`](../backend/app.py#L121) | 새 오류와 원래 원인을 연결한다 | `raise RuntimeError("실패") from e` | API 오류로 바꾸면서 원인 연결 보존 | except가 오류를 성공으로 바꾼다고 생각함 |
| `history or '(첫 대화)'` · [`backend/services.py:196`](../backend/services.py#L196) | 문맥이 비었으면 대체 문자열 | `"" or "처음"` → `"처음"` | 첫 질문에도 안내문 구성 | `or`가 None에만 적용된다고 생각함 |
| `x if condition else y` · [`backend/services.py:176`](../backend/services.py#L176) | 조건으로 두 값 중 하나 선택 | `"낮" if hour < 18 else "밤"` | Gemma/Llama 모델명 선택 | 두 모델을 동시에 호출한다고 해석함 |
| `lambda: fake_redis` · [`backend/test_app.py:60`](../backend/test_app.py#L60) | 인수 없이 객체를 돌려주는 짧은 함수 | `lambda: 3` | 테스트 의존성 교체 | 일반 앱에서도 FakeRedis가 쓰인다고 생각함 |
| `with st.sidebar`, `with st.form` · [`frontend/app.py:53`](../frontend/app.py#L53), [`frontend/app.py:106`](../frontend/app.py#L106) | 화면 요소를 놓을 영역·입력 제출 묶음 | `with st.form("f"): ...` | 화면 배치와 제출 시점 관리 | 모든 with가 DB 커밋을 뜻한다고 생각함 |
| `if __name__ == "__main__"` · [`init_database.py:75`](../init_database.py#L75) | 직접 실행한 경우만 관리 함수 호출 | `python tool.py` | import만으로 DB 초기화를 실행하지 않음 | 검증하려고 초기화 스크립트를 실행함 |
...
| 주석 도구 재실행 | 주석 대상 13개 | 15:43 KST에 모두 already_annotated, 추가 변경 없음 | 같은 해설이 중복 삽입되지 않음을 확인 |
| 최종 참조·표·다이어그램 구조 점검 | 소스 참조 134개·Markdown 표·두 그림 | 참조의 심벌과 최종 줄 내용 일치, 표 열 개수 일치, 노드·참여자 ID와 alt/end 균형 확인 | 전용 Mermaid 파서·렌더러 검증을 대신하지 않음 |
...
| 발견 사항 | 영향과 다음에 읽을 위치 |
| --- | --- |
| README의 기본 화면 포트 설명과 실제 기본 Compose가 다름 | README는 8501을 안내하지만 `compose.yml`은 Host 80을 공개한다. [`README.md:26`](../README.md#L26), [`compose.yml:54`](../compose.yml#L54). guide.md 106행에도 차이가 기록되어 있다. |
| 장애 교재의 오래된 health 키 | `TROUBLESHOOTING.md`의 `gemini_configured`와 현재 `checks.providers.gemini` 구조가 다름. [`TROUBLESHOOTING.md:99`](../TROUBLESHOOTING.md#L99), [`backend/app.py:75`](../backend/app.py#L75). |
| 채팅은 전체 원자적 작업이 아님 | 모델 실패면 user 행이, 후반 Redis 실패면 두 DB 행이 남을 수 있다. 재시도 중복 방지 키도 없다. [`backend/app.py:152`](../backend/app.py#L152), [`backend/services.py:129`](../backend/services.py#L129). |
...
| DB 초기화·테스트 의존성의 별도 준비 | `python-dotenv`, `pytest`가 Backend 명세에 없음. 초기화 설명은 [`guide.md:166`](../guide.md#L166), 테스트 구현은 `backend/test_app.py`. 이번에는 설치하지 않음. |
| 성공 채팅 직후 사용자 말풍선 갱신 | 입력 직후에는 답변만 명시적으로 렌더링하고 다음 실행의 GET에서 기록을 다시 읽음. [`frontend/app.py:79`](../frontend/app.py#L79)와 [`frontend/app.py:97`](../frontend/app.py#L97)를 비교한다. |
| 세션은 인증 수단이 아님 | GET·DELETE에 소유권 검사가 없고 세션별 동시 요청 직렬화도 없음. `get_chat_history`, `reset_current_session`과 `RedisSessionStore`를 읽는다. |
| 클라이언트 수명·오류 정보 처리 | Gemini 클라이언트는 지역 변수로 유지하지만 명시적 close가 없고, API 오류 detail에는 예외 문자열이 들어간다. 실제 누출을 확인한 것은 아니다. [`backend/services.py:226`](../backend/services.py#L226), `show_error`를 읽는다. |
