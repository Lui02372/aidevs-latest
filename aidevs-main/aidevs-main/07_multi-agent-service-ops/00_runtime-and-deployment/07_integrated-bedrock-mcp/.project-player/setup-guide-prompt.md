...
- 선택한 프로젝트 루트: `C:\수업 보관소\aidevs-latest\aidevs-main\aidevs-main\07_multi-agent-service-ops\00_runtime-and-deployment\07_integrated-bedrock-mcp`
...
```json
{
  ...
  "ancestor_ci": {
    "folder": "C:\\수업 보관소\\aidevs-latest\\aidevs-main\\aidevs-main",
    "workflows": [
      "C:\\수업 보관소\\aidevs-latest\\aidevs-main\\aidevs-main\\.github\\workflows\\07-runtime-ci.yml",
      "C:\\수업 보관소\\aidevs-latest\\aidevs-main\\aidevs-main\\.github\\workflows\\07-weather-mcp-cicd.yml",
      "C:\\수업 보관소\\aidevs-latest\\aidevs-main\\aidevs-main\\.github\\workflows\\07-weather-stateful-cicd.yml"
    ],
    "note": "Git 저장소 여부와 별개인 파일 근거입니다. 각 workflow의 paths와 working-directory가 선택 프로젝트에 적용되는지 읽어 확인하세요."
  }
}
```
...
선택 폴더를 먼저 `독립 실행 앱 / 여러 예제 모음 / 설명·배포 교재` 중 실제 내용에 맞게 분류하세요. Markdown·이미지만 있는 교재 폴더에 venv 생성이나 서버 명령을 만들어 넣지 마세요. 교재의 선행 실습과 실제 소스 경로가 명시되어 있으면 그 근거 파일을 연결하고, 직접 확인한 소스 경로와 교재에만 언급된 경로를 구분하세요. 확인 범위 밖의 소스는 미확인으로 표시하고 Project Player에서 다음에 선택할 폴더를 안내하세요. 클라우드 배포 교재는 로컬 실습과 계정·권한·사용자 설정이 필요한 배포 단계를 나누세요.
...
최종 응답은 가이드 **Markdown 본문만** 반환하세요. 전체 응답을 감싸는 코드 펜스, 작성 완료 인사, 파일을 직접 저장했다는 표현, 도구 호출 요청은 넣지 마세요. 실행 가능한 앱의 가이드에는 본문 안에 실제 구조에 근거한 `powershell` 코드 블록을 최소 하나 포함하세요. 설명·배포 교재만 있는 폴더에는 형식을 채우기 위해 실행 명령을 만들어 넣지 마세요. 프로젝트가 없거나 읽을 수 없는 부분은 해당 항목에 미확인 이유와 사용자의 다음 행동을 적고 나머지 확인 가능한 가이드는 완성하세요.

분석 JSON의 `ancestor_ci`에 경로가 있으면 Git 명령의 성공 여부와 무관하게 해당 workflow 파일을 읽으세요. ZIP으로 받은 교재도 상위 `.github/workflows`를 포함할 수 있습니다. 각 파일의 `paths`, `working-directory`, 테스트·빌드 대상을 확인해 선택 프로젝트에 해당하는 workflow만 연결하고, 다른 예제의 AWS 배포를 필수 준비로 섞지 마세요. 이 경로 제공은 상위 폴더 전체 탐색이나 변경을 허용하는 것이 아닙니다.

기존 guide.md의 네 단계 기본 틀과 `player` 실행 주석은 실행기가 과거에 만든 임시 안내일 수 있습니다. 그것을 사용자 작성 근거나 프로젝트 실행의 증거로 인용하지 마세요. 실제 README와 소스·설정에서 실행 순서를 새로 정하세요. 최종 문서에는 `player` 주석이나 기본 틀을 되풀이하지 마세요.

## 가이드의 필수 구성

최종 가이드는 한국어로 작성하고, 아래 **H2 제목 8개를 정확히 이 이름과 순서로 모두 사용하세요.** 제목은 `## 프로젝트와 실행 순서` 형식으로 쓰세요. 가이드 전체의 H1 제목은 추가해도 되지만, 다른 H2 제목을 만들거나 필수 제목을 바꾸지 마세요. 각 절의 세부 내용과 아래 번호 목록의 지시는 해당 H2 아래 H3(`###`)로 나누어 구성하세요.

| 필수 H2 제목 | 포함할 내용 |
| --- | --- |
| 프로젝트와 실행 순서 | 선택 폴더의 유형, 실제 구조와 구성 요소, 권장 진행 순서 |
| PowerShell | 필요한 도구, 작업 폴더, venv와 의존성, 로컬 실행·종료 명령 |
| 환경변수 | ENVEXAMPLE 복사, 변수별 의미와 필수 입력, 사용 주체와 설정 위치 |
| Docker | 실제 Compose 구성, 빌드·시작·확인·종료, 로컬 실행과의 관계 |
| GitHub Actions | 실제 workflow 근거, 연결할 대상과 권한·설정, CI 학습 순서 |
| 예제 학습 | 예제별 읽기·실행 순서, 화면에서 수행할 동작과 기대 결과, 다음 예제 |
| 검증과 문제 해결 | 확인 방법과 기대 결과, 증상별 해결 순서, 실제 실행 여부 |
| 근거와 미확인 사항 | 근거 파일·설정 키, 확인 범위, 미실행 사항과 남은 사용자 입력 |

**모든 절에는 비어 있지 않은 설명 본문이 있어야 합니다.** 도구나 기능이 해당되지 않으면 제목을 생략하지 말고 `해당 없음`과 실제 파일 구조에 근거한 이유를 적으세요. 확인할 수 없는 항목은 `미확인`과 이유·다음 확인 위치를 적으세요. 교재 전용 폴더에서는 실행 절차 대신 읽을 파일, 교재 진행 순서와 다음에 선택할 소스 폴더를 설명할 수 있습니다. 실제 실행 근거가 없는 venv·서버·workflow 명령을 만들어 넣지 마세요.

실행 단계마다 **목적 → 작업 폴더와 사용할 창 → 복사할 PowerShell 명령 → 예상 결과 → 실패 시 확인할 위치/다음 행동**을 제시하세요. 명령을 여러 대안과 한 블록에 뒤섞지 말고, 실제 구조에 맞는 권장 실행 경로 하나를 먼저 완성하세요. 대체 경로는 별도 표시하세요.

결과는 짧은 체크리스트나 README를 읽으라는 안내로 끝내지 마세요. 초보자가 이 문서 하나에서 준비 → 실행 → 화면에서 실습 → 결과 확인 → 종료 → 다음 예제로 이동까지 따라 할 수 있어야 합니다. 여러 예제가 있으면 실제 교재 순서에 따라 `순서 / 예제 경로 / 배울 내용 / 선행 조건 / 실행·확인 절차의 문서 내 위치` 표를 만들고, 각 실행 가능한 예제의 절차를 본문에 적으세요. 동일한 설치 절차는 앞 절을 참조해도 되지만 예제마다 달라지는 폴더·명령·환경변수·화면 동작·예상 결과를 생략하지 마세요. 실행할 코드가 없는 설명 장은 실행 불필요라고 명시하고 다음 실습과 연결하세요.

아래 8개 항목은 세부 내용 지시이며, 새로운 H2 목차가 아닙니다. 위의 필수 H2에 나누어 포함하고, 여러 예제를 따라 하는 실제 절차는 `예제 학습`에서 설명하세요.
...
첫 절에는 초보자가 그대로 따라갈 **하나의 번호 순서**를 먼저 적으세요. 각 번호에서 무엇을 왜 하고, 아래 어느 단계로 이동하는지 링크를 제공하세요. 이미 Project Player의 9번으로 서버가 실행 중인 학습자는 중복 실행하지 않고 접속·실습 단계로 이동하도록 설명하세요. 현재 서버 동작 여부를 직접 확인하지 않았다면 실행 중이라고 단정하지 마세요. 준비가 끝난 뒤 무슨 화면에서 무엇을 입력하고 결과가 어떻게 나와야 하는지 반드시 적으세요.
