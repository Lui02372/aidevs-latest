...
async def fake_guide():
    return "기온은 섭씨, 강수는 확률입니다."


def fake_generate_answer(provider: str, weather: dict, guide: str) -> tuple[str, str]:
...
def test_weather_agent_contract(monkeypatch) -> None:
    ...
    monkeypatch.setattr(backend_app, "read_weather_guide", fake_guide)
