...
SKILL_PATH = Path(__file__).parent / "skills" / "weather-briefing" / "SKILL.md"
RESOURCE_URI = "weather://guide/forecast"
app = FastAPI(title="07 Integrated Weather MCP Agent", version="1.0.0")
...
class StateStore:
    ...
    def cache_key(self, city, day):
        date = datetime.now(ZoneInfo("Asia/Seoul")).date().isoformat()
        return f"weather:cache:{date}:{city.strip().lower()}:{day}"
    def cached_weather(self, city, day):
        value = self.client.get(self.cache_key(city, day))
        return json.loads(value) if value else None
    def cache_weather(self, city, day, value):
        self.client.setex(self.cache_key(city, day), CACHE_TTL, json.dumps(value, ensure_ascii=False))
...
async def read_weather_guide():
    async with mcp_session() as session:
        result = await session.read_resource(RESOURCE_URI)
        text = "\n".join(item.text for item in result.contents if hasattr(item, "text"))
        if not text.strip(): raise RuntimeError("MCP resource가 비어 있습니다.")
        return text

def generate_answer(provider, weather, guide):
    ...
    if provider != "gemini": raise ValueError("지원하는 provider는 openai 또는 gemini입니다.")
...
async def ready():
    checks = {"redis": False, "database": False, "database_schema": False, "mcp": False, "resource": False, "skill": SKILL_PATH.is_file()}
    errors = {}
    ...
    try:
        async with mcp_session() as session:
            ...
            checks["resource"] = any(getattr(item, "text", "").strip() for item in resource.contents)
...
async def execute_weather_agent(run_id: str, payload: WeatherRequest):
    try:
        ...
        guide = await read_weather_guide()
        answer, model = await run_in_threadpool(generate_answer, payload.provider, tool_result, guide)
...
async def weather_agent(payload: WeatherRequest, background_tasks: BackgroundTasks):
    run_id = str(uuid4())
    state_store.progress(run_id, 5, "queued", "프로세스 내부 BackgroundTask를 예약했습니다.")
