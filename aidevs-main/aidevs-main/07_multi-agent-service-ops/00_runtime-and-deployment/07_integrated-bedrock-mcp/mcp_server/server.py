...
mcp = FastMCP("weather-tools", host="0.0.0.0", port=8010, stateless_http=True, json_response=True,
    transport_security=TransportSecuritySettings(enable_dns_rebinding_protection=True,
        allowed_hosts=["weather-mcp:8010", "127.0.0.1:8010", "localhost:8010"],
        allowed_origins=["http://weather-mcp:8010", "http://127.0.0.1:8010", "http://localhost:8010"]))

@mcp.resource("weather://guide/forecast")
def forecast_guide() -> str:
    """날씨 JSON의 단위와 해석 기준을 제공하는 읽기 전용 문서."""
    return "temperature_min/max는 섭씨, precipitation_probability는 0~100% 강수 확률입니다. date는 조회 도시의 현지 날짜입니다. 예보는 관측 확정값이 아니며 출처는 Open-Meteo입니다."
