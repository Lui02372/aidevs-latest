...
3. **프로젝트 설정과 인증을 준비합니다.** [환경변수](#환경변수)에서 `.env.example`을 복사하고 DB 비밀번호를 정합니다. 완성된 날씨 답변까지 확인하려면 선택한 OpenAI/Gemini API 키가 필요합니다.
...
브라우저 → Streamlit → FastAPI → MCP → Open-Meteo 순서로 날씨를 가져옵니다. 백엔드는 날씨 원자료, MCP resource, 로컬 skill 파일을 조합해 OpenAI 또는 Gemini에 전달합니다.
...
| AWS CLI v2 | EC2 배포 시 | README의 AWS 배포 준비 |
...
모델 이름은 **파일에 적힌 기본값**이며 현재 계정에서 호출 가능하다는 보장은 아닙니다. 전체 답변 경로에는 두 provider 중 하나의 외부 인증이 필요합니다. ChatGPT 로그인이나 구독 인증을 프로젝트 API 키로 사용할 수 없습니다.
...
### OpenAI 또는 Gemini API 키 준비

기본 provider는 OpenAI입니다. `.env`에 `OPENAI_API_KEY`를 입력하세요.
Gemini를 사용하려면 `GEMINI_API_KEY`를 입력하고 화면에서 `gemini`를 선택합니다.
로컬 앱은 AWS 자격 증명 없이 실행합니다. 선택한 provider의 모델 접근 가능 여부는 실제 요청으로 확인합니다.
...
위와 같은 Compose 실행 경로입니다. OpenAI·Gemini는 `.env`에 해당 API 키와 모델을 준비합니다. 인증 없이 시작하는 경우에는 화면의 전체 날씨 답변 대신 MCP probe까지만 실습하세요.

**목적:** 선택한 LLM의 API 키로 같은 앱을 실행합니다.
...
**예상 결과:** 두 프로젝트의 컨테이너가 제거되고 목록이 비어 있습니다. 종료에는 LLM API 키가 필요하지 않습니다.
...
| EC2 앱 설정 | 서버 설정 관리 권한 | GitHub Secret으로 전달하지 않음 | `/opt/weather-07/shared/.env` | EC2 배포 스크립트 | DB 설정·LLM API 키·모델 |
...
3. **도시**에 `Seoul`, **날짜**에 `내일`, **Cloud LLM**에 설정한 provider를 선택합니다. 기본 선택은 `openai`이며 Gemini 키를 준비했다면 `gemini`를 선택합니다.
4. **실제 날씨 조회**를 누릅니다.
...
readiness 성공은 **외부 Open-Meteo 호출이나 선택 LLM의 API 키·모델 호출 성공을 의미하지 않습니다.** 전체 완료 판단에는 화면에서 실제 조회가 필요합니다.
...
**목적:** 갱신된 LLM API 키·설정을 백엔드에 반영합니다.

**작업 폴더·창:** 프로젝트 루트, 새 인증을 준비한 창 A. 진행 중 작업이 없는 상태에서 실행합니다.

```powershell
docker compose -f compose.application.yml up -d --force-recreate --wait backend
Invoke-RestMethod -Uri 'http://127.0.0.1:8000/health/ready'
```

**예상 결과:** 백엔드가 새 환경으로 재생성되고 readiness가 성공합니다.

**실패 시:** 선택한 provider의 API 키와 인프라/MCP 상태를 확인하세요. 소스를 바꾼 경우 이미지도 다시 빌드합니다.
...
### 가이드 최초 작성 시의 확인 범위 (최신 검증은 VALIDATION.md 참고)
...
`VALIDATION.md`의 테스트 통과 기록은 기존 문서의 주장입니다. 해당 문서도 실제 Docker 화면 통합 및 EC2·GitHub CD·실제 LLM 검증은 미실행으로 구분합니다.
